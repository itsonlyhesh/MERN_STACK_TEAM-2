...
const Success = () => {
  ...
  useEffect(() => {
    if (!user) {
      navigate(`${MOVIE_BASE_PATH}/auth`);
      return;
    }
    if (!bookingId) {
      navigate(MOVIE_BASE_PATH);
      return;
    }
    ...
    const fetchBooking = async () => {
      try {
        const response = await axios.get(
          `${MOVIE_API_URL}/bookings/${bookingId}`,
          {
    ...
    <div className="page-container">
      <div className="success-receipt animate-fade-in">
        ...
        <div style={{ display: 'flex', gap: '16px', justifyContent: 'center' }}>
          <Link to={MOVIE_BASE_PATH} className="btn-secondary" style={{ padding: '10px 20px', fontSize: '0.85rem' }}>
            <ArrowLeft size={16} />
            <span>Go Home</span>
          </Link>
          <Link to={`${MOVIE_BASE_PATH}/my-bookings`} className="btn-primary" style={{ padding: '10px 20px', fontSize: '0.85rem' }}>
