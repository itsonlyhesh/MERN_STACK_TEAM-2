...
import { Clock, Calendar, Film, Monitor, Play, SlidersHorizontal } from 'lucide-react';
...
const MovieDetails = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const movieRes = await axios.get(`${MOVIE_API_URL}/movies/${id}`);
        setMovie(movieRes.data);

        const showsRes = await axios.get(`${MOVIE_API_URL}/shows?movieId=${id}`);
  ...
  const handleSelectShow = (show) => {
    selectShow(show);
    navigate(`${MOVIE_BASE_PATH}/seats`);
  };
